...
void isotp_on_can_message(IsoTpLink *link, uint8_t *data, uint8_t len)
{
    ...
    {
        isotp_user_debug("ISOTP_PCI_TYPE_SINGLE");
    ...
    {
        isotp_user_debug("ISOTP_PCI_TYPE_FIRST_FRAME");
        /* update protocol result */
        if (ISOTP_RECEIVE_STATUS_INPROGRESS == link->receive_status)
        {
            isotp_user_debug(" >ISOTP_PROTOCOL_RESULT_UNEXP_PDU");
            link->receive_protocol_result = ISOTP_PROTOCOL_RESULT_UNEXP_PDU;
        }
        else
        {
            isotp_user_debug(" >ISOTP_PROTOCOL_RESULT_OK");
            link->receive_protocol_result = ISOTP_PROTOCOL_RESULT_OK;
        }
        ...
        {
            isotp_user_debug(" >ISOTP_RET_OVERFLOW");
            /* update protocol result */
        ...
        if (ISOTP_RET_OK == ret)
        {
            isotp_user_debug(" >ISOTP_RECEIVE_STATUS_INPROGRESS");
            /* change status */
    ...
    {
        isotp_user_debug("ISOTP_PCI_TYPE_CONSECUTIVE_FRAME");
    ...
    case ISOTP_PCI_TYPE_FLOW_CONTROL_FRAME:
        isotp_user_debug("ISOTP_PCI_TYPE_FLOW_CONTROL_FRAME");
