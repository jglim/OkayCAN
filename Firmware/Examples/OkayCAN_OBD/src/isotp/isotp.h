...
typedef struct IsoTpLink {
    ...
    /* receiver paramters */
    uint32_t                    receive_arbitration_id_unused; // unused
    /* message buffer */
...
/**
 * @brief Prints the buffer's content into the active serial port
 * @param link Buffer to be printed
 * @param size Size of the buffer to be printed
 */
void printHex(uint8_t *buffer, uint16_t size);
